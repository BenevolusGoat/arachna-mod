...
function mod:spiderEggOrbitalInit(baby)
	...
	local rng = RNG()
	rng:SetSeed(baby.InitSeed, 35)
	sprite:ReplaceSpritesheet(0, "gfx/familiars/egg_orbital_" .. tostring(rng:RandomInt(4)+1) .. ".png")
	sprite:LoadGraphics()
...
function mod:spiderEggOrbitalTouch(baby, ent, _) 
	if baby:GetData().damageTimeout <= 0 then
		...
		if eggTryDie then
			local rng = RNG()
			local seed = math.floor((baby.Position.X + baby.Position.Y + ent.Position.X + ent.Position.Y + baby.InitSeed + ent.InitSeed)/6)
			rng:SetSeed(seed, 35)
			if (rng:RandomInt(5)+1 == 1) then
				--action
				local player = baby.Player
				local nearPos = Isaac.GetFreeNearPosition(baby.Position + Vector(mod:GetRandomNumber(-100, 100, mod.Globals.garbageRNG), mod:GetRandomNumber(-100, 100, mod.Globals.garbageRNG)), 50)
				...
				swirlEffect:Update()
				game:SpawnParticles(baby.Position, 5, mod:GetRandomNumber(3, 5, mod.Globals.garbageRNG), 4, Color(1, 1, 1, 1, 1, 1, 1))
...
function mod.spiderEggPickupUpdate(_, pickup)
	...
	if (game:GetRoom():IsClear()) then
		...
		swirlEffect.DepthOffset = 250
		swirlEffect:Update()
		game:SpawnParticles(pickup.Position, 5, mod:GetRandomNumber(3, 5, mod.Globals.garbageRNG), 4, Color(1, 1, 1, 1, 1, 1, 1))
...
function mod:arachnidGripsOnKill(ent)
	if (ent:IsEnemy()) and (ent.Type ~= EntityType.ENTITY_FIREPLACE) and (ent.SpawnerType == 0) and (ent.ParentNPC == nil) then --entity is dead and wasn't spawned by another entity
		...
		if (SomeoneHasItem(arachnidItem)) and (not game:GetRoom():IsClear()) then
			local rng = Isaac.GetPlayer(0):GetCollectibleRNG(arachnidItem)
			if (rng:RandomInt(100)+1 <= 20) then
		...
		for i=0, game:GetNumPlayers()-1 do
			...
			if (player:HasCollectible(arachnidItem, true)) and (rng:RandomInt(100)+1 <= 8) and (isFlyEnemy(ent))then
				player:AddHearts(2)
...
function mod:arachnidGripsPoison(tear)
	...
	if player then
		if (player:HasCollectible(arachnidItem, true)) then
			local rng = player:GetCollectibleRNG(arachnidItem)
