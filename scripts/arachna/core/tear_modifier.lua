...
function TearModifier.New(params)
	...
	---@return EntityTear|EntityBomb|EntityLaser|EntityKnife|EntityEffect
	local convertEnt = {
		[EntityType.ENTITY_TEAR] = function(ent) return ent:ToTear() end,
		[EntityType.ENTITY_BOMB] = function(ent) return ent:ToBomb() end,
		[EntityType.ENTITY_LASER] = function(ent) return ent:ToLaser() end,
		[EntityType.ENTITY_KNIFE] = function(ent) return ent:ToKnife() end,
		[EntityType.ENTITY_EFFECT] = function(ent) return ent:ToEffect() end,
	}
	...
	---@param damage number
	Mod:AddCallback(ModCallbacks.MC_POST_APPLY_TEARFLAG_EFFECTS, function(_, npc, pos, flags, sourceEnt, damage)
		if not sourceEnt or not TearModifier:IsValidEnemyTarget(npc) then
			return
		end
		local ent
		local convertFunc = convertEnt[sourceEnt.Type]
		if not convertFunc then
			return
		end
		ent = convertFunc(sourceEnt)
	...
	---@param sourceEnt Entity
	Mod:AddCallback(ModCallbacks.MC_POST_FIRE_SPLIT_TEAR, function(_, tear, sourceEnt, splitTearType)
		local data = getData(sourceEnt)
		local tearData = getData(tear)
		if data[modInitial .. self.Name] then
			tearData[modInitial .. self.Name] = true
		end
		if data[modInitial .. self.Name .. "_Disabled"] then
			tearData[modInitial .. self.Name .. "_Disabled"] = true
		end
	end)
